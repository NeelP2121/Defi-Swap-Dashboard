...
const CMCTableRow = ({
    starNum, coinName, coinIcon, coinSymbol = '---', price = '---',hRate = '---', dRate = '---', hRateIsIncrement, dRateIsIncrement, marketCapValue = '---', volumeValue = '---', volumeCryptoValue = '---', circulatingSupply = '---', 
}) => {
    const graphImages = [
    ...
    const getRandomGraphs = () => {
        console.log('generating graphs')
        const radInt = Math.floor(Math.random()*10)+1
        return graphImages[radInt]
    } 
    ...
    return (
        <tbody className={styles.tableRow}>
            <tr>
                ...
                <td>
                    <Image src={getRandomGraphs()} width={150} height={60} alt='graph'/>
                </td>
                <td>
                    <More />
                </td>
